const isValidPhoneNumber = require('../../lib/validators/isValidPhoneNumber')

describe('VALIDATORS:: #isValidPhoneNumber Test Suite', () => {
  it('should pass -- Happy Path', () => {
    try {
      expect(isValidPhoneNumber('236210021')).to.be.true
      expect(isValidPhoneNumber('210210021')).to.be.true
      expect(isValidPhoneNumber('911589233')).to.be.true
      expect(isValidPhoneNumber('924567232')).to.be.true
      expect(isValidPhoneNumber('934589239')).to.be.true
      expect(isValidPhoneNumber('961589237')).to.be.true
      expect(isValidPhoneNumber(236210021)).to.be.true
      expect(isValidPhoneNumber(210210021)).to.be.true
      expect(isValidPhoneNumber(911589233)).to.be.true
      expect(isValidPhoneNumber(924567232)).to.be.true
  ...
  it('should fail if not a String or a Number', () => {
    try {
      ...
      expect(err.details).to.be.eql('PhoneNumber must be a String or a Number.')
  ...
  it('should fail on Strings not only numerical', () => {
    try {
      ...
      expect(err.details).to.be.eql('PhoneNumber must have 9 digits, and start with 2 or 9.')
      expect(err).to.be.instanceof(ValidationError)
    }
  })
