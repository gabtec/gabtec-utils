const isValidNISS = require('../../lib/validators/isValidNISS')

describe('VALIDATORS:: #isValidNISS Test Suite', () => {
  it('should pass -- Happy Path', () => {
    try {
      expect(isValidNISS('11115611236')).to.be.true
      expect(isValidNISS('23456789233')).to.be.true
      expect(isValidNISS('13456789232')).to.be.true
      expect(isValidNISS(11115611236)).to.be.true
      expect(isValidNISS(23456789233)).to.be.true
      expect(isValidNISS(13456789232)).to.be.true
    } catch (err) {
  ...
  it('should fail on Strings not only numerical', () => {
    try {
      ...
      expect(err.details).to.be.eql('NISS must have 11 digits, and start with 1 or 2.')
      expect(err).to.be.instanceof(ValidationError)
    }
  })
