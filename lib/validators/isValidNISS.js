...
function isValidNISS (niss) {
  if (!(typeof niss === 'string' || typeof niss === 'Number')) {
    throw new ValidationError('Invalid NISS Type', 'NISS must be a String or a Number.')
  }
  ...
  if (!/^[1|2]\d{10}$/g.test(niss)) {
    throw new ValidationError('Invalid NISS Syntax', 'NISS must have 11 digits, and start with 1 or 2.')
  }

  const arrayNISS = niss.split('').reverse().map(sp => parseInt(sp, 10))
  ...
  if (control === arrayNISS[0]) {
    ...
    throw new ValidationError('Invalid NISS CheckDigit', 'Check digits do not match.')
  }
}
